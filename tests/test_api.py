...
@pytest.mark.asyncio
async def test_async_get_status_success(mock_aiohttp_session):
    """Test successful status fetch and XML parsing."""
    client = RixensApiClient(host="10.0.22.6", session=mock_aiohttp_session)
...
@pytest.mark.asyncio
async def test_async_get_status_empty_values(mock_aiohttp_session):
...
@pytest.mark.asyncio
async def test_async_get_status_nested_elements(mock_aiohttp_session):
...
@pytest.mark.asyncio
async def test_async_get_status_http_error(mock_aiohttp_session):
...
@pytest.mark.asyncio
async def test_async_get_status_invalid_xml(mock_aiohttp_session):
    """Test handling of invalid XML."""
    client = RixensApiClient(host="10.0.22.6", session=mock_aiohttp_session)
    
    with aioresponses() as mock:
        mock.get("http://10.0.22.6/status.xml", status=200, body="Not valid XML {{{")
...
@pytest.mark.asyncio
async def test_async_get_status_timeout(mock_aiohttp_session):
    """Test handling of timeout."""
    client = RixensApiClient(host="10.0.22.6", session=mock_aiohttp_session, timeout=0.1)
...
@pytest.mark.asyncio
async def test_async_set_value_success(mock_aiohttp_session):
...
@pytest.mark.asyncio
async def test_async_set_value_http_error(mock_aiohttp_session):
...
@pytest.mark.asyncio
async def test_async_set_value_timeout(mock_aiohttp_session):
    """Test handling of timeout when setting value."""
    client = RixensApiClient(host="10.0.22.6", session=mock_aiohttp_session, timeout=0.1)
    
    with aioresponses() as mock:
        mock.get("http://10.0.22.6/interface.cgi?act=101&val=680", exception=TimeoutError())
...
@pytest.mark.asyncio
async def test_host_normalization(mock_aiohttp_session):
...
@pytest.mark.asyncio
async def test_value_coercion(mock_aiohttp_session):
